...
type UserInfo struct {
	...
	LastPunished int64 // 8 bytes

	// Padding to 64 bytes (current: 60 bytes, need 4 more)
	_ [4]byte
}
...
// GetArenaStats returns cache statistics for monitoring
// Returns: hits, misses, collisions
func GetArenaStats() (uint64, uint64, uint64) {
	// Stub implementation - return zeros for now
	// Can be enhanced with actual tracking if needed
	return 0, 0, 0
}
